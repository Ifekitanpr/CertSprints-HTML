...
const targetDir = path.join(webRoot, "practice");

const pages = [
  "practice-exam.html",
  "practice-exam-session.html",
];

const redirectPage = (file) => {
  const target = `practice/${file}`;
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>CertSprints Practice</title>
  <script>
    location.replace("${target}" + location.search + location.hash);
  </script>
</head>
<body>
  <p><a href="${target}">Continue to Practice</a></p>
</body>
</html>
`;
};
...
for (const file of pages) {
  ...
  if (source.includes(`location.replace("practice/${file}"`)) {
  ...
  console.log(`Migrated ${file} -> web/practice/${file}`);
}

console.log("Practice migration complete.");
