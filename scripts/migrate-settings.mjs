...
const targetDir = path.join(webRoot, "settings");

const pages = [
  "settings.html",
  "settings-account.html",
  "settings-adjust-plan.html",
  "settings-delete-account.html",
  "settings-learning-plan.html",
  "settings-notifications.html",
  "settings-security.html",
  "settings-security-password.html",
  "settings-sounds.html",
];

const redirectPage = (file) => {
  const target = `settings/${file}`;
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>CertSprints Settings</title>
  <script>
    location.replace("${target}" + location.search + location.hash);
  </script>
</head>
<body>
  <p><a href="${target}">Continue to Settings</a></p>
</body>
</html>
`;
};
...
for (const file of pages) {
  ...
  if (source.includes(`location.replace("settings/${file}"`)) {
  ...
  console.log(`Migrated ${file} -> web/settings/${file}`);
}
